...
- 完成后**自动导入书库**；在书库中按 `<书名>-<bilinovelID>.epub` 命名（重命名安全字符；同一本书重复下载会**原地替换**，保留阅读进度）；产出的 EPUB 按卷分组目录（TOC）
- **更新**：搜索列表/详情页按 `u`（或书库 `u`、CLI `lnreadertui update <id|标题>`）增量更新已下载的书——只抓取**新发表**的章节/卷并追加到本地 EPUB 末尾（旧内容与封面/插图原样保留，阅读进度与章节索引不受影响）；站点目录与本地不一致时安全中止
...
# 更新：把已下载的书更新到网站最新进度（只追加新章节）
lnreadertui update "玩乐关系"             # → 书库内文件原地更新，进度保留
lnreadertui update 4649 --json
...
| 书库 | `enter` 阅读 · `u` 更新（追加新章节） · `i` 导入 · `d` 删除 · `x` 重置进度 · `/` 过滤 |
| 搜索 | 输入即搜 · `enter` 立即搜索 · `↓` 聚焦结果 · `enter` 打开详情 · `u` 更新书库中已有的书 |
| 详情 | `↑/↓ j/k pgup/pgdn` 滚动 · `enter` 下载 · `u` 更新已有书 · `esc` 返回 |
| 下载 | `x` 取消 · `d` 移除 |
